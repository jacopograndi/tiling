use crate::quad_snd::{AudioContext, PlaySoundParams};
...
impl MixerControl {
    // only accepts 2 channel f32 [-1.0..1.0] samples at 44KHz
    pub fn load(&self, samples: &[f32]) -> u32 {
        let sound_id = self.sound_id.get();

        self.tx
            .send(crate::quad_snd::mixer::AudioMessage::AddSound(
                sound_id,
                samples.to_vec(),
            ))
