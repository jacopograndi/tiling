...
use crate::quad_snd::{error::Error, PlaySoundParams};

use quad_alsa_sys as sys;

use std::sync::mpsc;

pub use crate::quad_snd::mixer::Playback;

mod consts {
    pub const DEVICES: &[&str] = &["default\0", "pipewire\0"];
    pub const RATEMIN: u32 = 44100;
    pub const RATEMAX: u32 = 48000;
...
unsafe fn setup_pcm_device() -> *mut sys::snd_pcm_t {
    ...
    // Set hardware parameters
    {
        ...
        let mut ratemin = consts::RATEMIN;
        let mut ratemax = consts::RATEMAX;
        if sys::snd_pcm_hw_params_set_rate_minmax(
            pcm_handle,
            hw_params,
            &mut ratemin,
            std::ptr::null_mut(),
        ...
        let mut rate = ratemin.max(ratemax);
        ...
        // TODO: pass this rate to the asset pcm resampler?
...
unsafe fn audio_thread(mut mixer: crate::quad_snd::mixer::Mixer) {
...
pub struct AudioContext {
    pub(crate) mixer_ctrl: crate::quad_snd::mixer::MixerControl,
}

impl AudioContext {
    pub fn new() -> AudioContext {
        use crate::quad_snd::mixer::Mixer;
...
impl Sound {
    pub fn load(ctx: &AudioContext, samples: &[f32]) -> Sound {
        let sound_id = ctx.mixer_ctrl.load(samples);
