...
extern "C" {
    fn audio_init();
    fn audio_add_buffer_raw(content: *const f32, content_len: u32) -> u32;
...
impl Sound {
    pub fn load(_ctx: &AudioContext, data: &[f32]) -> Sound {
        let buffer = unsafe { audio_add_buffer_raw(data.as_ptr(), data.len() as u32) };
        Sound(buffer)
    }
